...
onmessage = function (e) {
    ...
    switch (msg.type) {
        ...
        case 'start': {
            //msg里面包含webm_metadata
            metadata = msg.webm_metadata;
            //去除msg里面包含的msg_options
            options = {
            ...
            webm_muxer.onmessage = function (e) {
                ...
                switch (msg2.type) {
                    case 'ready':
                        ...
                        //发送这个信号的时候，向运行encoder-worker.js的worker发送消息
                    case 'start-stream':
                        send_metadata(metadata);
                        break;

                    case 'exit':
                        //如果消息的 type 属性为 exit，则 muxer 已完成（所有轨道已完成，muxer 已刷新其缓冲区并发送回所有已混合数据）。
                        webm_muxer.terminate();
                        self.postMessage(msg2);
                        break;

                    case 'muxed-data':
                        //发送这个信号的时候，发送的消息的data属性要包含webm输出的下一个块
                        self.postMessage(msg2, [msg2.data]);
                        break;

                    case 'stats':
                        //如果消息的 type 属性是 stats 则 data 属性包含一个具有以下属性的对象：
                        self.postMessage(msg2);
                        break;
